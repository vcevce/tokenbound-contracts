...
import "../mocks/MockAccountModifiedUpgradable.sol";

contract AccountTest is Test {
    Multicall3 forwarder;
    AccountV3Modified implementation;
    AccountV3ModifiedUpgradable upgradableImplementation;
    AccountProxy proxy;
    ...
    function setUp() public {
        ...
        implementation = new AccountV3Modified(
            address(forwarder), address(registry), address(guardian)
        );
        upgradableImplementation = new AccountV3ModifiedUpgradable(
            address(forwarder), address(registry), address(guardian)
        );
    ...
    function testAccountLocksAndUnlocks() public {
        ...
        // unlocking account should change state
        assertTrue(state != account.state());

        // unlocking should fail if account is already unlocked
        vm.prank(user1);
        ...
        // signing should now that account is unlocked
    ...
    function testExecuteNested() public {
        ...
        AccountV3Modified nestedAccount = AccountV3Modified(payable(accountAddress3));
    ...
    function testAccountOwnerIsNullIfContextNotSet() public {
        ...
        assertEq(AccountV3Modified(payable(accountClone)).owner(), address(0));
    }
    ...
    function testAccountUpgrade() public {
        ...
        AccountV3ModifiedUpgradable account = AccountV3ModifiedUpgradable(payable(accountAddress));

        MockAccountModifiedUpgradable upgradedImplementation = new MockAccountModifiedUpgradable(
            address(1),
            address(1),
            address(1)
        );
        ...
        vm.prank(user1);
        account.upgradeTo(address(upgradedImplementation));
        uint256 returnValue = MockAccountModifiedUpgradable(payable(accountAddress)).customFunction();
